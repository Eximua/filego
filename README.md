...
Filego 总控客户端可以直达 Filego 生态内的所有产品，其中包括：云盘客户端、钱包客户端、区块浏览器、节点浏览器、节点状态监控浏览器。

### 身份认证

在 Filego 生态内有两种用户身份：矿工和用户（开发者也算用户），这两种身份是互相隔离的。用户在 Filego 总控客户端完成注册后，可以进行文件的管理、上传、下载、移动、分享等操作。矿工通过启动 Filego 客户端可以管理自己的节点、钱包、查看区块和全网节点运行状态，这些信息都是公开的。

用户注册完成后，会取得自己的私钥和公钥，其中公钥就是用户的 FiCoin 钱包地址，私钥需要用户保存在安全的地方，Filego 不会存储。注册成为 Filego 的用户是中心化的管理方式，这是为了提升身份认证的效率、速度和安全性，Filego 除了保存用户的基本账户信息外不会保存其它任何信息。Filego 会使用结合 HTTPS, JWT, MD5, 非对称加密和 Salt 等多种方式保证用户的信息安全，并对服务器读写权限严格控制。
